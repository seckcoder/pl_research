...
; macro expander
; Note this version of macro expander has a minor bug for cond(because of the support of 
; guard for pattern matcher). 
(require "../cps/builtin.rkt"
         "../compiler/scheme/base.rkt")
...
(define (expand x [boundvars (seteq)])
  (let ([expand-with-boundvars
    ...
    (match x
      ...
      [`(,rator ,rand* ...)
        ; function call
        `(,(expand-with-boundvars rator) ,@(map expand-with-boundvars rand*))]
